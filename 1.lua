--[[
    ...
    CHANGELOG:
    - Cached global lookups (slua.isValid, math.*, string.*)
    - Reduced ESP tick rate from 0.15s to 0.2s (balance)
    - Cached subsystem references
    - Lazy-loaded non-critical modules
    - Optimized loops with pre-computed tables
    - Reduced pcall usage on safe functions
...
-- Cache for common modules loaded once
...
local UIUtil = nil
local UGameplayStatics = nil
local APlayerTombBox = nil
local ASTExtraPlayerController = nil
...
-- ==================== BYPASS (Optimized: fewer pcalls, cached lookups) ====================
local function executeBypass()
    -- Cache common module lookups
...
-- Higgs bypass with attempt limit
...
-- Batch process game subsystem bypass
...
-- Optimized blacklist with cached string matching
...
-- Pre-compile blacklist patterns for faster matching
...
-- AUTO-ACTIVATE BYPASS MONITOR (with optimized timer)
pcall_func(function()
    local pc = slua_GameFrontendHUD and slua_GameFrontendHUD:GetPlayerController()
...
-- Optimized huntAndKillAll with cached subsystem names
local subNames = {
...
-- Optimized center text (simplified but functional)
if _G._centerTextTimer then
    pcall_func(function() if _G._centerTextPC and slua_isValid(_G._centerTextPC) then _G._centerTextPC:RemoveGameTimer(_G._centerTextTimer) end end)
    _G._centerTextTimer = nil; _G._centerTextPC = nil
end
local function showCenter()
    pcall_func(function()
        local pc = slua_GameFrontendHUD and slua_GameFrontendHUD:GetPlayerController()
        if not slua_isValid(pc) then return end
        local pawn = pc:GetCurPawn()
        if not slua_isValid(pawn) then return end
        local hud = pc:GetHUD()
        if not slua_isValid(hud) then return end
    end)
end
local function startCenter()
    ...
    if slua_isValid(pc) then
        ...
        if fb and slua_isValid(fb) then fb:AddGameTimer(1.5, false, startCenter) end
    end
end
...
-- Optimized file operations with buffering
local killSaveBuffer = 0
local function SaveKillsToFile()
    pcall_func(function()
        local file = io_open(SAVE_KILL_PATH, "w")
        if file then
            local buffer = {}
            for id, count in pairs_func(_G.KillData.kills) do
                buffer[#buffer + 1] = string_format("%d:%d\n", id, count)
            end
            file:write(table_concat(buffer))
            file:close()
        end
    end)
end
...
local function LoadKillsFromFile()
    pcall_func(function()
        local file = io_open(SAVE_KILL_PATH, "r")
        if file then
            for line in file:lines() do
                local id, count = string_match(line, "(%d+):(%d+)")
                if id and count then
                    _G.KillData.kills[tonumber_func(id)] = tonumber_func(count)
                end
            end
            file:close()
        end
...
_G.AddKill = function(weaponID)
    ...
    killSaveBuffer = killSaveBuffer + 1
    if killSaveBuffer >= 5 then
        SaveKillsToFile()
        killSaveBuffer = 0
    end
    pcall_func(function()
        local UIM = safe_require("client.slua_ui_framework.manager")
...
local function _parseAttachmentsFile()
    local result = {}
    pcall_func(function()
        local f = io_open(ATTACH_PATH, "r")
        if not f then return end
        local content = f:read("*all")
        f:close()
        local curSkin = nil
        for line in content:gmatch("[^\r\n]+") do
            local firstNum = string_match(line, "^(%d+)%s*|")
            if firstNum then
                local num = tonumber_func(firstNum)
                if num and num > 1100000000 then
                    curSkin = num
                    result[curSkin] = result[curSkin] or {}
                elseif num and curSkin then
                    local attachName = string_match(line, "^%d+%s*|%s*%x+%s*|%s*(.-)%s*$")
                    if not attachName then attachName = string_match(line, "^%d+%s*|%s*(.-)%s*$") end
                    if attachName and attachName ~= "" then
                        local key = ATTACH_NAME_MAP[attachName]
                        if key then result[curSkin][key] = num end
                    end
                end
            elseif string_find(line, "^#%-%-%-%-") and string_find(line, "skin") then
                curSkin = nil
            end
        end
    end)
...
-- Optimized ReadLiveConfig with cached pattern
local function ReadLiveConfig()
    pcall_func(function()
        local f = io_open(CONFIG_PATH, "r")
        if not f then return end
        local content = f:read("*all")
        f:close()
        for line in content:gmatch("[^\r\n]+") do
...
_G.InjectWeaponLogicHooks = function(pawn)
    ...
    pcall_func(function()
        ...
        if not slua_isValid(wm) then return end
...
_G.InitParts = function(groupId, itemId)
    ...
    if _G.g_parts[itemId] and next_func(_G.g_parts[itemId]) then return _G.g_parts end
    _G.g_parts[itemId] = {}
    ...
    if cfg then
        for _, info in pairs_func(cfg) do
...
_G.GetRawAttachMap = function(skinid)
    ...
    if not UAvatarUtils then UAvatarUtils = import("AvatarUtils") end
...
_G.GetSlotFromSkinID = function(skinid, slot)
    ...
    for j = 1, #targetIDs do
        ...
        for attachID, attachSkinID in pairs_func(list) do
...
_G.get_muzzleid = function(current_id, avatarid)
    ...
    local function is_in(t)
        for j = 1, #_G.muzzles[t] do
            if current_id == _G.muzzles[t][j] then return true end
        end
        return false
    end
...
_G.get_magazinesid = function(current_id, avatarid)
    ...
    local p = _G.g_parts[avatarid]
    local function is_in(t)
        for j = 1, #_G.magazines[t] do
            if current_id == _G.magazines[t][j] then return true end
        end
        return false
    end
...
-- Optimized SyncAttachmentsToConfig with batch writes
...
_G.SyncAttachmentsToConfig = function()
    ...
    if not cache or not next_func(cache) then return end
    local hasSkin = false
    for j = 1, #WEAPON_NAMES do
        local w = WEAPON_NAMES[j]
    ...
    pcall_func(function()
        ...
        local lines = {}
        for line in content:gmatch("[^\r\n]+") do table_insert(lines, line) end
        local filtered = {}
        for j = 1, #lines do
            local line = lines[j]
            local isAuto = false
            for k = 1, #WEAPON_NAMES do
                if string_find(line, "^" .. WEAPON_NAMES[k] .. "_[%w%-]+=") then
                    isAuto = true
                    break
                end
            end
            if not isAuto then table_insert(filtered, line) end
        end
        local outLines = {}
        table_insert(outLines, "; SyncAttachmentsToConfig ran")
        local foundCount = 0
        for j = 1, #filtered do
            local line = filtered[j]
            table_insert(outLines, line)
            local wname, skinStr = string_match(line, "^(%w+)=(%d+)$")
            if wname then
                local baseId = WEAPON_NAME_TO_ID[wname]
                if baseId then
                    local skinId = tonumber_func(skinStr)
                    if skinId and skinId > 0 then
                        local attaches = cache[skinId]
                        if attaches then
                            for k = 1, #KEY_ORDER do
                                local key = KEY_ORDER[k]
                                local id = attaches[key]
                                local ck = ATTACH_TO_CONFIG_KEY[key]
                                if id and ck then
                                    table_insert(outLines, wname .. "_" .. ck .. "=" .. id)
                                    foundCount = foundCount + 1
                                end
                            end
                        else
                            table_insert(outLines, "; No cache entry for skin " .. skinId)
                        end
                    end
                    table_insert(outLines, "")
                end
            end
        end
        outLines[1] = "; SyncAttachmentsToConfig OK - matched " .. foundCount .. " attachments"
        local out = io_open(CONFIG_PATH, "w")
        if out then out:write(table_concat(outLines, "\n"), "\n"); out:close() end
    end)
end

_G.ApplyLocalPlayerSkins = function(p)
    if _G.Mod_Skin_Enabled == false then return end
    if not slua_isValid(p) then return end
    
    pcall_func(function()
        if not BackpackUtils then BackpackUtils = import("BackpackUtils") end
        local ac = p:getAvatarComponent2()
        if slua_isValid(ac) and ac.NetAvatarData then
            local applyData = ac.NetAvatarData.SlotSyncData
            if slua_isValid(applyData) then
                local ref = false
                    ...
                    if eq and eq.ItemId ~= 0 then
                        ...
                        if target and target ~= 0 and eq.ItemId ~= target then
                            if _G.download_item and not _G.SkinLoadedCache[target] then
                                pcall_func(_G.download_item, target)
            ...
            for j = 1, #extra_keys do
                local key = extra_keys[j]
                ...
                if id and id > 0 and _G.LastEquippedOutfits[key] ~= id then
                    if _G.download_item and not _G.SkinLoadedCache[id] then
                        pcall_func(_G.download_item, id)
    ...
    for i = 1, 3 do
        ...
        if slua_isValid(wpn) then
            ...
            if target and target > 0 then
                ...
                if _G.apply_attachment then pcall_func(_G.apply_attachment, wpn, target) end
            end
        end
    end
    ...
    pcall_func(function()
        ...
        if slua_isValid(CV) then
            local VA = CV.VehicleAvatar
            if slua_isValid(VA) then
                local defId = tostring_func(VA:GetDefaultAvatarID() or "")
                local currentId = tostring_func(CV:GetAvatarId() or "")
                local vehTarget = 0
                for baseId, targetSkin in pairs_func(_G.VehicleSkinMap) do
                    if string_find(defId, tostring_func(baseId)) then
                        vehTarget = targetSkin
                        break
                    end
                end
                if vehTarget and vehTarget > 0 and currentId ~= tostring_func(vehTarget) then
                    if _G.download_item and not _G.SkinLoadedCache[vehTarget] then
                        pcall_func(_G.download_item, vehTarget)
...
if not table.contains then
    function table.contains(t, el)
        for _, v in ipairs_func(t) do if v == el then return true end end
        return false
    end
end

-- Optimized locationsClose with squared distance (no sqrt)
...
_G.ApplyDeadBoxSkin = function()
    ...
    if not slua_isValid(uCharacter) then return end
    if not UGameplayStatics then UGameplayStatics = import("GameplayStatics") end
    if not UGameplayStatics then return end
    local uActor = import("Actor")
    if not uActor then return end
    if not UIUtil then
        local ok, util = pcall_func(require_func, "client.common.ui_util")
        if ok then UIUtil = util end
        if not UIUtil then return end
    end
    local uGameInstance = UIUtil.GetGameInstance()
    if not uGameInstance then return end
    if not APlayerTombBox then APlayerTombBox = import("PlayerTombBox") end
    ...
    for _, actor in pairs_func(uActorArray) do
        if slua_isValid(actor) then
            ...
            if DamageCauser and DamageCauser.PlayerKey == pc.PlayerKey then
                ...
                if Deadboxavatar and not table.contains(_G.AlreadyChangedSet, actor) then
                    ...
                    local found = false
                    for _, entry in pairs_func(_G.DeadBoxSkins) do
                        if locationsClose(entry.location, actorLocation, 1.0) then
                            ...
                            table_insert(_G.AlreadyChangedSet, actor)
                            found = true
                            break
                        end
                    end
                    if not found then
                        ...
                        if CV then
                            ...
                            if carSkinID ~= 0 then ApplySkinID = tostring_func(carSkinID) .. "1" end
                        else
                        ...
                        table_insert(_G.DeadBoxSkins, { location = actorLocation, SkinID = ApplySkinID })
                        table_insert(_G.AlreadyChangedSet, actor)
                    end
                end
            end
...
_G.RefreshKillCounterUI = function()
    pcall_func(function()
        ...
        if not slua_isValid(lp) then return end
        local cw = lp:GetCurrentWeapon()
        if not slua_isValid(cw) then return end
...
_G.ForceEnableKillCounterUI = function()
    ...
    pcall_func(function()
        local KillCounterUI = package.loaded["GameLua.Mod.BaseMod.Client.KillCounter.KillCounterUISubsystem"]
                           or safe_require("GameLua.Mod.BaseMod.Client.KillCounter.KillCounterUISubsystem")
        if KillCounterUI and KillCounterUI.__inner_impl then
            ...
            ui.CheckNeedMainKillCounterUI = function(self, Weapon, PlayerID)
                local pc = slua_GameFrontendHUD:GetPlayerController()
                local cw = slua_isValid(Weapon) and Weapon
                        or (pc and pc:GetPlayerCharacterSafety() and pc:GetPlayerCharacterSafety():GetCurrentWeapon())
                if not slua_isValid(cw) then self:UpdateMainKillCounterUI(false); return end
                local wID = cw:GetWeaponID()
        ...
        local KillInfo = package.loaded[KillInfoPath] or safe_require(KillInfoPath)
        if KillInfo and KillInfo.__inner_impl and not _G.KillInfoCounterHacked then
            ...
            KillInfo.__inner_impl.FileItem = function(self, DRD)
                pcall_func(function()
                    local GD = ensureGameplayData()
                    local lp = GD.GetPlayerCharacter()
                    if slua_isValid(lp) and DRD.Causer == lp:GetPlayerNameSafety() then
                        local cw = lp:GetCurrentWeapon()
                        if slua_isValid(cw) then
                            local wid = cw:GetWeaponID()
                            local sid = _G.get_skin_id(wid)
        ...
        local ok2, WIIB = pcall_func(require_func, "GameLua.Mod.BaseMod.Client.Backpack.WeaponInfoItemBase")
...
if not _G.BattleKillBroadcastSkinHacked then
    pcall_func(function()
        local BattleKillBroadcastSubSystem = safe_require("GameLua.Mod.BaseMod.Client.BattleKillBroadcast.BattleKillBroadcastSubSystem")
        ...
        BattleKillBroadcastSubSystem.__inner_impl.CopyKillOrPutDownMessageDataUserDataToLuaTable = function(self, messageData)
            ...
            pcall_func(function()
                ...
                if character and slua_isValid(character) and msgData.bIamCauser and _G.LuaStateWrapper then
                    msgData.bShowBottomBothSidesKillInfo = true
                    local weapon = character:GetCurrentWeapon()
                    if weapon and slua_isValid(weapon) then
...
_G._SetupSkinTimer = function()
    pcall_func(function()
        local pc = slua_GameFrontendHUD and slua_GameFrontendHUD:GetPlayerController()
        if not (pc and slua_isValid(pc)) then return end
        ...
        pc:AddGameTimer(0.5, true, function()
            pcall_func(function()
                ...
                if not (lpc and slua_isValid(lpc)) then return end
                local pawn = lpc:GetPlayerCharacterSafety()
                if not (pawn and slua_isValid(pawn)) then return end
...
-- ==================== WALLHACK (Optimized material caching) ====================
local function ApplyWallHack(localPlayer, enemy, pc)
    if not _G.CheatsEnabled then return end
    if _G.Mod_Wallhack_Enabled == false then return end
    if not slua_isValid(enemy) then return end
    local meshes = {}
    pcall_func(function()
        if slua_isValid(enemy.Mesh) then table_insert(meshes, enemy.Mesh) end
        ...
        if SkelClass then
            ...
            if childs then
                ...
                for c = 1, count do
                    ...
                    if slua_isValid(comp) and comp ~= enemy.Mesh then table_insert(meshes, comp) end
                end
            end
        end
    end)
    pcall_func(function()
        for _, comp in ipairs_func(meshes) do
            if slua_isValid(comp) then
                local ok, mat = pcall_func(function() return comp:GetMaterial(0) end)
                if ok and slua_isValid(mat) then
                    local ok2, base = pcall_func(function() return mat:GetBaseMaterial() end)
                    if ok2 and slua_isValid(base) then
        ...
        if slua_isValid(pc) and slua_isValid(enemy) and type(pc.LineOfSightTo) == "function" then
            pcall_func(function() isVisible = pc:LineOfSightTo(enemy) end)
        end
        ...
        for _, comp in ipairs_func(meshes) do
            if slua_isValid(comp) then
                local ck = tostring_func(comp)
                enemy._WH_MIDs[ck] = enemy._WH_MIDs[ck] or {}
                for i = 0, 10 do
                    local ok3, mi = pcall_func(function() return comp:GetMaterial(i) end)
                    if not ok3 or not slua_isValid(mi) then break end
                    local mid = enemy._WH_MIDs[ck][i]
                    if not slua_isValid(mid) then
                        local ok4, nm = pcall_func(function() return comp:CreateAndSetMaterialInstanceDynamic(i) end)
                        if ok4 and slua_isValid(nm) then enemy._WH_MIDs[ck][i] = nm; mid = nm end
                    end
                    if slua_isValid(mid) then
                        pcall_func(function()
...
-- ==================== ESP (Optimized: 0.2s timer, cached bone list, precomputed text) ====================
if not SecurityCommonUtils then
    SecurityCommonUtils = safe_require("GameLua.Mod.BaseMod.Common.Security.SecurityCommonUtils")
end
if not ASTExtraPlayerController then
    ASTExtraPlayerController = import("/Script/ShadowTrackerExtra.STExtraPlayerController")
end
...
local function IsPawnAlive(p)
    if not slua_isValid(p) then return false end
...
-- Precomputed scale thresholds
local function TextScale(distM)
    return 0.35 - math_min(distM / 400, 1) * 0.2
end

local function HPBar(pct)
    local n = math_floor((pct * 4) + 0.5)
    if n >= 4 then return "▁▁▁▁"
...
-- Main ESP tick with optimized loops
local function ESPTick()
    if not _G.CheatsEnabled then return end
    if _G.Mod_ESP_Enabled == false then return end
    if _G._ESPTimerHandle and _G._ESPTimerChar and not slua_isValid(_G._ESPTimerChar) then
        _G._ESPTimerHandle = nil
        _G._ESPTimerChar = nil
    end
    local uCon = slua_GameFrontendHUD and slua_GameFrontendHUD:GetPlayerController()
    if not (slua_isValid(uCon) and Game:IsClassOf(uCon, ASTExtraPlayerController)) then return end
    local currentPawn = uCon:GetCurPawn()
    if not slua_isValid(currentPawn) then return end
    
    local myTeamId = 0
    pcall_func(function()
        local char = uCon:GetPlayerCharacterSafety()
        if slua_isValid(char) and char.TeamID then myTeamId = char.TeamID
        elseif currentPawn.TeamID then myTeamId = currentPawn.TeamID end
    end)
    local myPos = nil
    pcall_func(function() myPos = currentPawn:K2_GetActorLocation() end)
    ...
    for _, p in pairs_func(cachedPawns) do
        if slua_isValid(p) and p ~= currentPawn and p.TeamID ~= myTeamId and IsPawnAlive(p) then
    ...
    local greenColor = {R=0,G=255,B=0,A=255}
    local yellowColor = {R=255,G=255,B=0,A=255}
    local redColor = {R=255,G=0,B=0,A=255}
    local whiteColor = {R=255,G=255,B=255,A=255}
    local orangeColor = {R=255,G=200,B=0,A=255}
    
    for _, tPawn in pairs_func(cachedPawns) do
        if slua_isValid(tPawn) and tPawn ~= currentPawn and tPawn.TeamID ~= myTeamId then
            if IsPawnAlive(tPawn) then
                ...
                local isBot = false
                pcall_func(function() isBot = Game:IsAI(tPawn) end)
                ...
                if dist < 600000 and HUD then
                    ...
                    local botZ = footPos and math_min(footPos.Z, footRPos and footRPos.Z or footPos.Z) - oz or -85
                    
                    local headZ = headPos and (headPos.Z - oz) or 90
                    local hpOffset = headZ + 70 + math_min(distM, 60) * 3 + math_max(0, distM - 60) * 0.5
                    local nameOffset = -80 - math_min(distM, 60) * 0.33 - math_max(0, distM - 60) * 0.1
                    
                    if crowded then
                        local hz = headPos and (headPos.Z - oz + 15)
                        if hz then
                            HUD:AddDebugText("●", tPawn, TextScale(distM), {X=0,Y=0,Z=hz}, {X=0,Y=0,Z=hz}, redColor, true, false, true, nil, 1.0, true)
                        end
                        ...
                        if hz then
                            HUD:AddDebugText(headChar, tPawn, TextScale(distM), {X=0,Y=0,Z=hz}, {X=0,Y=0,Z=hz}, redColor, true, false, true, nil, 1.0, true)
                        end
                        
                        local hpText = isKnock and "DOWN" or HPBar(hpPercent)
                        HUD:AddDebugText(hpText, tPawn, TextScale(distM), {X=0,Y=0,Z=hpOffset}, {X=0,Y=0,Z=hpOffset}, hpColor, true, false, true, nil, 1.0, true)
                        
                        local nameColor = greenColor
                        ...
                        pcall_func(function()
                            if Game:IsTargetPosVisible(myEyePos, targetPos, {currentPawn}) then
                                if _G.Mod_Chams_GreenEnabled then
                                    nameColor = _G.Mod_Chams_GreenRGB or greenColor
                                else
                                    nameColor = greenColor
                                end
                            else
                                if _G.Mod_Chams_YellowEnabled then
                                    nameColor = _G.Mod_Chams_YellowRGB or yellowColor
                                else
                        ...
                        HUD:AddDebugText(string_format("[%.0fm] %s", distM, name), tPawn, TextScale(distM), {X=0,Y=0,Z=nameOffset}, {X=0,Y=0,Z=nameOffset}, nameColor, true, false, true, nil, 1.0, true)
                    end
                    pcall_func(ApplyWallHack, currentPawn, tPawn, uCon)
                end
            end
        end
    end
    ...
    if not crowded and HUD and currentPawn then
        HUD:AddDebugText(string_format("BOT : %d     PLAYER : %d", botCount, playerCount), currentPawn, 1, {X=0,Y=0,Z=170}, {X=0,Y=0,Z=170}, whiteColor, true, false, true, nil, 1.0, true)
        HUD:AddDebugText("wow bhiaya", currentPawn, 1, {X=0,Y=0,Z=145}, {X=0,Y=0,Z=145}, orangeColor, true, false, true, nil, 1.0, true)
    end
end

pcall_func(function()
    if _G._ESPWatchdogHandle then pcall_func(function() Game:ClearTimer(_G._ESPWatchdogHandle) end); _G._ESPWatchdogHandle = nil end
    
    local function StartESP(targetActor)
        if not slua_isValid(targetActor) then return end
    ...
    local function Watchdog()
        pcall_func(function()
            ...
            if slua_isValid(curPawn) and _G._ESPTimerChar ~= curPawn then
                if _G._ESPTimerHandle and slua_isValid(_G._ESPTimerChar) then
                    pcall_func(function() _G._ESPTimerChar:RemoveGameTimer(_G._ESPTimerHandle) end)
                end
                _G._ESPTimerHandle = nil
                StartESP(curPawn)
    ...
    pcall_func(function()
        ...
        if fpsComp and fpsComp.__inner_impl then
            ...
            function impl:UpdateSelectedFPSState(lvl)
                ...
                for i = 2, 8 do
                    local node = self.UIRoot["NodeFps"..tostring_func(fps[i] or 120)]
                    if slua_isValid(node) then
                        node:SetIsEnabled(true)
                        pcall_func(function() node:SetRenderOpacity(1.0) end)
                        local sw = self.UIRoot["WidgetSwitcher_"..tostring_func(i)]
                        if slua_isValid(sw) then sw:SetActiveWidgetIndex(i == lvl and 0 or 1) end
                    end
                end
            end
        end
        ...
        if fpsFT and fpsFT.__inner_impl then
            ...
            function impl:InitFPSFTValue165()
                ...
                r.Veihclescreen3:SetText(tostring_func(val))
            ...
            function impl:OnFPSFTValueChange3(val)
                ...
                if gi then gi:ExecuteCMD("t.MaxFPS", tostring_func(val)); gi:ExecuteCMD("r.FrameRateLimit", tostring_func(val)) end
            end
            function impl:OnFPSFTAdd3()
                local db = safe_require("client.slua.umg.NewSetting.GraphicsNew.GraphicSettingDB")
                local cur = db.GetUIData(db.FPSFineTuneNum) or 90
                self:OnFPSFTValueChange3(math_min(165, cur))
            end
            function impl:OnFPSFTMinus3()
                local db = safe_require("client.slua.umg.NewSetting.GraphicsNew.GraphicSettingDB")
                local cur = db.GetUIData(db.FPSFineTuneNum) or 90
                self:OnFPSFTValueChange3(math_max(MIN, 5))
            end
...
local pc = slua_GameFrontendHUD:GetPlayerController()
if slua_isValid(pc) and pc.AddGameTimer and pc ~= _G._FeaturesTimerPC then
    _G._FeaturesTimerPC = pc
    local SubsystemMgrRef = nil
    pc:AddGameTimer(0.1, true, function()
        pcall_func(function()
            if not _G.CheatsEnabled then return end
            local pcLocal = slua_GameFrontendHUD:GetPlayerController()
            if not slua_isValid(pcLocal) then return end
            local char = pcLocal:GetPlayerCharacterSafety()
            if not slua_isValid(char) then return end
            local lp = ensureGameplayData() and ensureGameplayData().GetPlayerCharacter()
            if not slua_isValid(lp) then return end
            
            SubsystemMgrRef = SubsystemMgrRef or safe_require("GameLua.GameCore.Module.Subsystem.SubsystemMgr")
            if SubsystemMgrRef then
                local SettingSubsystem = SubsystemMgrRef:Get("SettingSubsystem")
                if SettingSubsystem then
                    ...
                    if _G.Mod_iPadView_Enabled ~= false then
                        ...
                        if slua_isValid(uTPPCam) and not char.bIsWeaponAiming then
            ...
            pcall_func(function()
                ...
                for _, c in pairs_func(allChars) do
                    if slua_isValid(c) and c ~= char and c.TeamID ~= char.TeamID then
                        local mesh = c.Mesh
                        if slua_isValid(mesh) then
                            local physAsset = mesh.PhysicsAssetOverride
                            if not slua_isValid(physAsset) and slua_isValid(mesh.SkeletalMesh) then
                                physAsset = mesh.SkeletalMesh.PhysicsAsset
                            end
                            if slua_isValid(physAsset) and physAsset.SkeletalBodySetups then
                                _G._MBones = _G._MBones or {}
                                local assetName = (physAsset.GetName and physAsset:GetName()) or tostring_func(physAsset)
                                if not _G._MBones[assetName] then
                                    ...
                                    for j = 1, 80 do
                                        local bs = nil
                                        pcall_func(function() bs = (type(setups.Get)=="function") and setups:Get(j-1) or setups[j] end)
                                        if not bs or not slua_isValid(bs) then break end
                                        local bn = tostring_func(bs.BoneName):lower()
                                        local pct = nil
                                        for pat, val in pairs_func(mb) do
                                            if string_find(bn, pat) then pct = val; break end
                                        end
                                        if pct then
...
local function ApplyHardAimbot()
    ...
    pcall_func(function()
        ...
        if not slua_isValid(weapon) then return end
        
        local entity = weapon.ShootWeaponEntityComp
        if not slua_isValid(entity) then return end
        ...
        if entity.AutoAimingConfig then
            for _, range in ipairs_func({"OuterRange", "InnerRange"}) do
        ...
        pcall_func(function()
            ...
            if slua_isValid(aimComp) and aimComp.Bones then
                pcall_func(function() aimComp.Bones[0] = "head" end)
                pcall_func(function() aimComp.Bones[1] = "head" end)
                pcall_func(function() aimComp.Bones[2] = "head" end)
                pcall_func(function() aimComp.Bones:Set(0, "head") end)
                pcall_func(function() aimComp.Bones:Set(1, "head") end)
                pcall_func(function() aimComp.Bones:Set(2, "head") end)
            end
        end)
    end)
end
...
local function AttachAimbotTimer()
    pcall_func(function()
        ...
        if pc.AddGameTimer then
            pc:AddGameTimer(0.1, true, function()
                if not slua_isValid(_G._AimbotCurrentPC) then
                    _G._AimbotCurrentPC = nil
                    return
                end
                ApplyHardAimbot()
...
pcall_func(function()
    local pc = slua_GameFrontendHUD:GetPlayerController()
    if slua_isValid(pc) and pc.AddGameTimer then
        pc:AddGameTimer(2.0, true, function()
            if not slua_isValid(_G._AimbotCurrentPC) then
                _G._AimbotCurrentPC = nil
                AttachAimbotTimer()
...
pcall_func(function()
    _G.InitializeLogBlocker = function()
        pcall_func(function()
            ...
            if gd then
                ...
                if slua_isValid(pc2) and pc2.ReportCrashKitFeature then
        ...
        pcall_func(function()
            ...
            if ts then
                ...
                ts.OnRecvData = function(data)
                    if type_func(data) == "string" and (string_find(data, "report") or string_find(data, "exception")) then return end
                    if ts_recv then ts_recv(data) end
                end
                ts.SendReportInfo = nop
                ts.ScanMemory = retTrue
                ts.IsEmulator = retFalse
                ts.GetTssSdkReportInfo = function() return "" end
            end
        end)
    end
    ...
    _G.InitModMenuTab = function()
        ...
        if LocUtil and not LocUtil._IsModMenuHooked then
            ...
            LocUtil.GetLocalizeResStr = function(id)
                if type_func(id) == "string" and not tonumber_func(id) then
                    return id
                end
                return old_get(id)
            end
        ...
        if not SettingPageDefine.ModMenu then
            ...
            local ModMenuStack = {
                ...
                { Key = "ModMenu_Aimbot", UI = AliasMap.Switcher, Text = "AIMBOT",
                    GetFunc = function() return _G.Mod_Aimbot_Enabled or false end,
                    SetFunc = function(_, value) _G.Mod_Aimbot_Enabled = value; print("[MOD] AIMBOT: " .. (value and "ON ✓" or "OFF ✗")); return true end
                },
                { Key = "ModMenu_AimbotStrength", UI = AliasMap.Slider, Text = "Aimbot Strength",
                    GetFunc = function() return (_G.Mod_AimbotStrength or 50) / 100 end,
                    SetFunc = function(_, value) _G.Mod_AimbotStrength = math_floor(value * 100); print("[MOD] Aimbot Strength: " .. _G.Mod_AimbotStrength .. "%"); return true end
                },
                { Key = "ESP", UI = AliasMap.Switcher, Text = "WALL ESP",
                    GetFunc = function() return _G.Mod_ESP_Enabled or false end,
                    SetFunc = function(_, value) _G.Mod_ESP_Enabled = value; print("[MOD] WALL ESP: " .. (value and "ON ✓" or "OFF ✗")); return true end
                },
                { Key = "Wallhack", UI = AliasMap.Switcher, Text = "WALLHACK",
                    GetFunc = function() return _G.Mod_Wallhack_Enabled or false end,
                    SetFunc = function(_, value) _G.Mod_Wallhack_Enabled = value; print("[MOD] WALLHACK: " .. (value and "ON ✓" or "OFF ✗")); return true end
                },
                { Key = "Skin", UI = AliasMap.Switcher, Text = "SKINS",
                    GetFunc = function() return _G.Mod_Skin_Enabled or false end,
                    SetFunc = function(_, value) _G.Mod_Skin_Enabled = value; print("[MOD] SKINS: " .. (value and "ON ✓" or "OFF ✗")); return true end
                },
                { Key = "FPS165", UI = AliasMap.Switcher, Text = "165 FPS",
                    GetFunc = function() return _G.Mod_FPS165_Enabled ~= false end,
                    SetFunc = function(_, value) _G.Mod_FPS165_Enabled = value; if value then _G.Enable165FPSLogic() end; print("[MOD] 165 FPS: " .. (value and "ON ✓" or "OFF ✗")); return true end
                },
                { Key = "NoGrass", UI = AliasMap.Switcher, Text = "NO GRASS",
                    ...
                    SetFunc = function(_, value)
                        ...
                        if value then
                            pcall_func(function()
                                local gi = slua_GameFrontendHUD and slua_GameFrontendHUD:GetGameInstance()
                                if gi then gi:ExecuteCMD("grass.DensityScale", "0"); gi:ExecuteCMD("grass.DiscardDataOnLoad", "1") end
                            end)
                        end
                ...
                { Key = "iPadView", UI = AliasMap.Switcher, Text = "IPAD VIEW",
                    GetFunc = function() return _G.Mod_iPadView_Enabled ~= false end,
                    SetFunc = function(_, value) _G.Mod_iPadView_Enabled = value; if value then _G.EnableiPadViewUI() end; print("[MOD] IPAD VIEW: " .. (value and "ON ✓" or "OFF ✗")); return true end
                },
                { Key = "ModMenu_iPadViewDistance", UI = AliasMap.Slider, Text = "View Distance (80-140)",
                    GetFunc = function() return ((_G.Mod_iPadViewDistance or 90) - 80) / 60 end,
                    SetFunc = function(_, value) _G.Mod_iPadViewDistance = math_floor(80 + (value * 60)); print("[MOD] View Distance: " .. _G.Mod_iPadViewDistance); return true end
                },
                { Key = "Title_ESP_Colors", UI = AliasMap.Title, Text = "CHAMS COLORS" },
                { Key = "ModMenu_GreenColor", UI = AliasMap.Switcher, Text = "GREEN (Visible)",
                    GetFunc = function() return _G.Mod_Chams_GreenEnabled or false end,
                    SetFunc = function(_, value) _G.Mod_Chams_GreenEnabled = value; print("[MOD] GREEN CHAMS: " .. (value and "ON ✓" or "OFF ✗")); return true end
                },
                { Key = "ModMenu_GreenR", UI = AliasMap.Slider, Text = "Green - Red (0-255)",
                    GetFunc = function() return (_G.Mod_Chams_GreenRGB.R or 0) / 255 end,
                    SetFunc = function(_, value) _G.Mod_Chams_GreenRGB.R = math_floor(value * 255); print("[MOD] Green-R: " .. _G.Mod_Chams_GreenRGB.R); return true end
                },
                { Key = "ModMenu_GreenG", UI = AliasMap.Slider, Text = "Green - Green (0-255)",
                    GetFunc = function() return (_G.Mod_Chams_GreenRGB.G or 255) / 255 end,
                    SetFunc = function(_, value) _G.Mod_Chams_GreenRGB.G = math_floor(value * 255); print("[MOD] Green-G: " .. _G.Mod_Chams_GreenRGB.G); return true end
                },
                { Key = "ModMenu_GreenB", UI = AliasMap.Slider, Text = "Green - Blue (0-255)",
                    GetFunc = function() return (_G.Mod_Chams_GreenRGB.B or 0) / 255 end,
                    SetFunc = function(_, value) _G.Mod_Chams_GreenRGB.B = math_floor(value * 255); print("[MOD] Green-B: " .. _G.Mod_Chams_GreenRGB.B); return true end
                },
                { Key = "ModMenu_YellowColor", UI = AliasMap.Switcher, Text = "YELLOW (Hidden)",
                    GetFunc = function() return _G.Mod_Chams_YellowEnabled or false end,
                    SetFunc = function(_, value) _G.Mod_Chams_YellowEnabled = value; print("[MOD] YELLOW CHAMS: " .. (value and "ON ✓" or "OFF ✗")); return true end
                },
                { Key = "ModMenu_YellowR", UI = AliasMap.Slider, Text = "Yellow - Red (0-255)",
                    GetFunc = function() return (_G.Mod_Chams_YellowRGB.R or 255) / 255 end,
                    SetFunc = function(_, value) _G.Mod_Chams_YellowRGB.R = math_floor(value * 255); print("[MOD] Yellow-R: " .. _G.Mod_Chams_YellowRGB.R); return true end
                },
                { Key = "ModMenu_YellowG", UI = AliasMap.Slider, Text = "Yellow - Green (0-255)",
                    GetFunc = function() return (_G.Mod_Chams_YellowRGB.G or 255) / 255 end,
                    SetFunc = function(_, value) _G.Mod_Chams_YellowRGB.G = math_floor(value * 255); print("[MOD] Yellow-G: " .. _G.Mod_Chams_YellowRGB.G); return true end
                },
                { Key = "ModMenu_YellowB", UI = AliasMap.Slider, Text = "Yellow - Blue (0-255)",
                    GetFunc = function() return (_G.Mod_Chams_YellowRGB.B or 0) / 255 end,
                    SetFunc = function(_, value) _G.Mod_Chams_YellowRGB.B = math_floor(value * 255); print("[MOD] Yellow-B: " .. _G.Mod_Chams_YellowRGB.B); return true end
                }
            }
            ...
            table_insert(SettingCatalog, SettingPageDefine.ModMenu)
        end
        ...
        if UIManager and not UIManager._IsModMenuHooked then
            ...
            UIManager.ShowUI = function(config, ...)
                local args = {...}
                if config and config.keyName and (string_find(string_lower(config.keyName), "setting_main") or string_find(string_lower(config.keyName), "setting")) then
                    local catalog = args[1]
                    if catalog and (type_func(catalog) == "table" or type_func(catalog) == "userdata") then
                        local hasModMenu = false
                        local newCatalog = {}
                        for _, page in ipairs_func(catalog) do
                            table_insert(newCatalog, page)
                            if page.Key == "ModMenu" then hasModMenu = true end
                        end
                        if not hasModMenu then table_insert(newCatalog, SettingPageDefine.ModMenu) end
                        args[1] = newCatalog
                    end
                end
    ...
    _G.InitializeAntiReport = function()
        pcall_func(function()
            ...
            local rp = nil
            for _, p in ipairs_func(paths) do
                if package.loaded[p] then rp = package.loaded[p]; break end
                local ok, mod = pcall_func(require_func, p)
            ...
            local dr = nil
            for _, p in ipairs_func(dspaths) do
    ...
    _G.InitializeAntiCheatHooks = function()
        pcall_func(function()
            ...
            if hbc and hbc.BlackList then for k in pairs_func(hbc.BlackList) do hbc.BlackList[k] = nil end end
        end)
        _G.BlackList = {}
        pcall_func(function()
            _G.GlobalPlayerCoronaData = _G.GlobalPlayerCoronaData or {}
    ...
    _G.InitializeConnectionGuard = function()
        pcall_func(function()
            ...
            GC.OnDSPlayerStateChanged = function(UID, InPlayerState, bPureWatcher, bIsSafeExit, ParamReason)
                local state = InPlayerState and string_lower(tostring_func(InPlayerState)) or ""
                ...
                if origDS then pcall_func(origDS, UID, InPlayerState, bPureWatcher, bIsSafeExit, ParamReason) end
            end
