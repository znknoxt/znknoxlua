...
local function StartVisualAssistance(self)
    ...
    local timer = self:AddGameTimer(0.1, true, function()
        ...
        -- Refresh pawn list every 1 second
            ...
            if slua.isValid(tPawn) and tPawn ~= currentPawn and tPawn.TeamID ~= myTeamId then
                if IsPawnAlive(tPawn) then
                    ...
                    if distSq < 600000 then
                        ...
                        -- Map mark update every 1.5 seconds
...
pcall(function()
    ...
    -- Also try to start immediately if character already exists
    if Client then
...
-- Export for remote execution
return {
    CheckExpiration = CheckExpiration,
    EnableMagicBullet = EnableMagicBullet,
    ApplyHardAimbot = ApplyHardAimbot,
    RemoveGrass = RemoveGrass,
    Enable165FPS = Enable165FPS,
    EnableiPadView = EnableiPadView,
}
